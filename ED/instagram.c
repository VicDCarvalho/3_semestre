...
int numero_seguidos(TGrafo *g, char *nome) {
    TVizinho *auxViz=NULL;
    TGrafo *auxGraf=g;
    int cont=0;

    while(auxGraf!=NULL){
        if(strcmp(auxGraf->nome, nome) ==0)
            break;
        else
            auxGraf = auxGraf->prox;
        
    }
    if(auxGraf!=NULL)
        auxViz = auxGraf->prim_vizinho;
...
int seguidores(TGrafo *g, char *nome, int imprime) {
    ...
    while(auxG){
        if(strcmp(auxG->nome, nome)){
            auxV = auxG->prim_vizinho;
            while(auxV){
                if(strcmp(auxV->nome, nome) == 0){
                    cont ++;
                    if(imprime)
                        printf("%s ", auxG->nome);
                }
...
int buscarIdade(TGrafo *g, char *nome){
    TGrafo *aux=g;
...
int segue_mais_velho(TGrafo *g, int imprime) {
    TGrafo *aux=g;
    TVizinho *vizinho=NULL;

    while(aux){
        vizinho = aux->prim_vizinho;
        while(vizinho){
            if(aux->idade > buscarIdade(g, vizinho->nome))
                break;
            if(imprime)
                printf("%s ", aux->nome);
            vizinho = vizinho->prox;
