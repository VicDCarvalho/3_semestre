...
int ehNaoOrientado(TGrafo *g) {
    ...
    while(aux){
        viz = aux->prim_vizinho;
        while(viz){
            if(viz->id_vizinho > aux->id_vertice){
                TGrafo *aux2 = busca_vertice(g, viz->id_vizinho);
                TVizinho *viz2 = busca_vizinho(aux2->prim_vizinho, aux->id_vertice);
                if(viz2== NULL || viz2->peso != viz->peso){
                    return 0;
                }
            }
            viz = viz->prox;
