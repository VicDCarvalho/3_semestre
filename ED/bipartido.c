...
TGrafo *buscaVertice(TGrafo *g, int id){
    TGrafo *aux=g;

    while(aux){
        if(aux->id_vertice == id)
            return aux;
        aux = aux->prox;
    }
    return NULL;
...
int ehbipartido(TGrafo *g) {
    ...
    while(aux){
        ...
        while(!pilha_vazia(pilha)){
            int atual = pop(pilha);
            TGrafo *aux2=buscaVertice(g, atual);
            viz = aux2->prim_vizinho;
            while(viz){
                TGrafo *aux3 = buscaVertice(g, viz->id_vizinho);
                if(aux3->cor == -1){
                    aux3->cor = 1-aux2->cor;
                    push(pilha, aux3->id_vertice);
                }
                if(aux3->cor == aux2->cor)
                    return 0;
                viz = viz->prox;
            }
        }
